...
yate.Codegen = function(lang, filename) {
    this.lang = lang;
    this._templates = {};
...
yate.Codegen.prototype._addTemplate = function(id, template) {
    var templates = this._templates[id] || (( this._templates[id] = [] ));
    templates.push(template);
};

//  Возвращаем все шаблоны для данного id.
yate.Codegen.prototype._getTemplate = function(id) {
...
yate.Codegen.prototype._readTemplates = function(filename) {
    ...
    for (var i = 0, l = parts.length; i < l; i++) {
        ...
        if (predicate) {
            ...
            //  Отрезаем '[' и ']'.
            predicate = new Function('ast', 'return !!(' + predicate + ');' );
        }
...
yate.Codegen.prototype.generate = function(name, ast, mode) {
    var suffix = ':' + (mode || '');
...
yate.Codegen.prototype._doTemplate = function(template, ast) {

    //  Если есть предикат, проверяем его истинность.
    var predicate = template.predicate;
    if ( predicate && !predicate(ast) ) {
        return;
    }
...
//
yate.Codegen.prototype._doLine = function(line, ast) {
    var r = line.split(/(\s*%{.*?})/);
...
yate.Codegen.prototype._doMacro = function(macro, ast) {
