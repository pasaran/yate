    ...
    for (var i0 = 0, l0 = nodeset.length; i0 < l0; i0++) {
        ...
        while (!found && i < p) {
            ...
            if (names) {
                ...
                if (templates) {
                    var j = 0;
                    var q = templates.length;
                    while (!found && j < q) {
                        var tid = templates[j++];
                        var template = module[tid];

                        //  В template.s лежит id селектора (jpath'а).
                        if ( module.matched(template.s, c0, i0, l0) ) {
                            found = tid;
                        }
                    }
                }
        ...
        if (found) {
            ...
            if (args) {
                ...
                r += module[found].apply( module, [c0, i0, l0, a0].concat(args) );
            } else {
                r += module[found](c0, i0, l0, a0);
            }
        }
    }
...
Module.prototype.matched = function matched(jid, c0, i0, l0) {
    if (jid === null) {
        //  Это jpath '/'
        return !c0.parent;
    }
    ...
    while (i >= 0) {
        ...
        if (step === 0) {
            ...
            var pid = jpath[i + 1];
            if ( !this[pid](c0, i0, l0) ) { return false; }
...
Module.prototype.select = function select(jid, context) {
    var jpath = this[jid];

    var current, m;
    //  FIXME: Эту проверку нужно убрать!
    ...
    for (var i = 0, n = jpath.length; i < n; i += 2) {
        ...
        switch (type) {
            ...
            case 2: // Это filter, в step предикат.
                for (var j = 0; j < m; j++) {
                    var node = current[j];
                    if (this[step](node, j, m)) { // Предикат принимает три параметра: node, index и count.
                        result.push(node);
                    }
                }
                break;
            ...
            case 3: // Это index, в step индекс нужного элемента.
                var node = current[ this[step](node) ];
                result = (node) ? [ node ] : [];
                break;
...
Module.prototype.vars = function vars(name, c0) {
    var value = this.vars[name];
    if (value === undefined) {
        value = this.vars[name] = this[name](c0, 0, 1);
    }
    return value;
};

Module.prototype.key = function(id, use, c0) {
    var cache = this._keys[id];
    var nodes, key;

    if (!cache) {
        cache = this._keys[id] = {};
        cache.xml = this[id + 'bt'] === 'xml';

        var key = this[id];
        var items = this[id + 'n'](c0);
        var nodes = cache.nodes = {};
        var a0 = {};
        for (var i0 = 0, l0 = items.length; i0 < l0; i0++) {
            var c1 = items[i0];
            var use_ = this[id + 'u'](c1, i0, l0);
            if (this[id + 'ut'] === 'nodeset') {
                //  Тип use_ -- nodeset.
                for (var j = 0, m = use_.length; j < m; j++) {
                    var c2 = use_[j];
                    nodes[ this.nodeValue(c2) ] = {
                        c: c1,
                        i: i0,
                        l: l0
                    };
    ...
    var node = nodes[use];
    if (cache.xml) {
        return (node) ? this[id + 'b']( node.c, node.i, node.l, {} ) : '';
    } else {
        return (node) ? this[id + 'b']( node.c, node.i, node.l ) : [];
    }
};
