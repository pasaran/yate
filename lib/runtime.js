...
function apply(nodeset, mode, result, _) {
        ...
        for (var i = 0, l = templatesList.length; i < l; i++) {
            ...
            if (matched(template.jpath, context, index, count)) {
                if (_) {
                    ...
                    template.body.apply(null, [ context, result, index, count ].concat(args));
                } else {
                    template.body(context, result, index, count);
                }
                break;
            }
        }
...
function closeAttrs(result) {
    var attrs = result.attrs;
    var buf = result.buf;
    for (var attr in attrs) {
        buf.push(' ' + attr + '="' + attrQuote(attrs[attr]) + '"');
    }
    buf.push('>');
    result.open = false;
};
