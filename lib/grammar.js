...
rules.block = function(p, a) {
    ...
    while ( !( input.isEOF() || this.testAny( [ '}', ']', ')' ] ) ) ) {
        ...
        var r = null;
        if ( this.test('IMPORT') ) {
            Imports.add( this.match('import') );

        } else if ( this.test('INCLUDE') ) {
            a.mergeWith( this.match('include').p.Block );

        } else if ( this.test('MATCH') ) {
            Templates.add( this.match('template') );

        } else if ( this.test('KEY') ) {
            Defs.add( this.match('key') );

        } else if ( this.test('FUNC') ) {
            Defs.add( this.match('function_') );

        } else if ( this.test('EXTERNAL') ) {
            Defs.add( this.match('external') );

        } else if ( this.testAll( [ 'QNAME', '=' ] ) ) {
            Defs.add( this.match('var_') );

        } else {
            Exprs.add( this.match('block_expr') );
...
rules.include = function() {
    ...
    return yate.parse(path_.join(dirname, filename), 'module');
};
...
yate.grammar = new Grammar(grammar);
