...
rules.block = function(p, a) {
    var input = this.input;

    var Imports = p.Imports.p;
    var Defs = p.Defs.p;
    var Templates = p.Templates.p;
    var Exprs = p.Exprs.p;
    ...
    while ( !( input.isEOF() || this.testAny( [ '}', ']', ')' ] ) ) ) {
        ...
        var r = null;
        if ( this.test('IMPORT') ) {
            Imports.push( this.match('import') );

        } else if ( this.test('INCLUDE') ) {
            a.mergeWith( this.match('include').p.Block );

        } else if ( this.test('MATCH') ) {
            Templates.push( this.match('template') );

        } else if ( this.test('KEY') ) {
            Defs.push( this.match('key') );

        } else if ( this.test('FUNC') ) {
            Defs.push( this.match('function_') );

        } else if ( this.test('EXTERNAL') ) {
            Defs.push( this.match('external') );

        } else if ( this.testAll( [ 'QNAME', '=' ] ) ) {
            Defs.push( this.match('var_') );

        } else {
            Exprs.push( this.match('block_expr') );
...
rules.import = function(p, a) {
    this.match('IMPORT');
    p.Name = this.match( 'inline_string', { noexpr: true } );
};
...
rules.arglist = function(p, a) {
    ...
    if ( this.test('arglist_item') ) {
        p.push( this.match('arglist_item') );
        while ( this.test(',') ) {
            this.match(',');
            p.push( this.match('arglist_item') );
        }
...
//  if_ := 'if' inline_expr body else_if* else_?

rules.if_ = function(p, a) {
    this.match('IF');
    p.Condition = this.match('inline_expr');
    p.Then = this.match('body');
    var Elses = p.Elses.p;
    while ( this.test('ELSE IF') ) {
        Elses.push( this.match('else_if') );
    }
    if ( this.test('ELSE') ) {
        Elses.push( this.match('else_') );
    }
};

//  else_if := 'else if' inline_expr body
...
rules.for_ = function(p, a) {
    this.match('FOR');
    p.Selector = this.match('inline_expr');
    p.Body = this.match('body');
};
...
rules.attr = function(p, a) {
    ...
    if ( this.test('{') ) {
        this.match('{');
        p.Name = this.match('inline_expr');
        this.match('}');
    } else {
        p.Name = a.make( 'string_literal', this.match('QNAME') );
    }
    ...
    while ( input.current() && !this.test(params.delim) ) {
        if ( !params.noexpr && this.test('{') ) {
            if ( this.test('{{') ) {
                ...
                if (s) {
                    p.push( a.make('string_literal', s) );
                    s = '';
                }
                this.match('{');
                this.skip('spaces');
                p.push( a.make( 'string_expr', this.match('inline_expr') ) );
                this.skip('spaces');
                this.match('}');
            }
