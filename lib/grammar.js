...
rules.template = function(ast) {
    ...
    ast.Selectors.add( this.match('jpath') );
    while ( this.test('|') ) {
        this.match('|');
        ast.Selectors.add( this.match('jpath') );
    }
...
rules.if_ = function(ast) {
    ...
    while ( this.test('ELSE IF') ) {
        ast.Elses.add( this.match('else_if') );
    }
    if ( this.test('ELSE') ) {
        ast.Elses.add( this.match('else_') );
    }
};

//  else_if := 'else if' inline_expr body

rules.else_if = function(ast) {
    this.match('ELSE IF');
    ast.Condition = this.match('inline_expr');
    ast.Body = this.match('body');
};
...
rules.sort = function(ast) {
    this.match('SORT');
    this.match('(');
    ast.Nodes = this.match('inline_expr');
    this.match(',');
    ast.By = this.match('inline_expr');
    if ( this.test(',') ) {
        this.match(',');
        ast.Order = this.match('inline_expr');
    }
    this.match(')');
};
