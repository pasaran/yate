...
// body = inlineExpr | '{' block '}' | '[' block ']' | blockExpr

Yate.Grammar.rules.body = function(ast) {

    if (this.testAll([ 'inlineExpr', 'EOL' ])) { // EOL на конце обязательный, потому что может быть так:
                                                 // for item if something foo

                                                 // FIXME: Тут не может быть inlineScalar, хотя и очень хочется его сюда поставить.
                                                 // Контрпример такой: if something foo else bar
                                                 // В этом случае else не будет воспринято как ключевое слово и получится: if something (foo else bar)
        return this.match('inlineExpr');
    }
    ...
    if (start) {
        ...
        ast.Type = (start == '{') ? 'value' : 'list';

        var end = (start == '{') ? '}' : ']';
        this.match(end);
    } else {
        return this.match('blockExpr');
    }
};
...
// blockExpr = if_ | for_ | apply | attr | xmlLine | array | object | list
...
Yate.Grammar.rules.list = function(ast) {
    ast.add( this.match('listItem') );
    while (this.test(',')) {
        this.match(',');
        ast.add( this.match('listItem') );
    }
...
// listItem = pair | scalar

Yate.Grammar.rules.listItem = function(ast) {
    if (this.test('pair')) {
        return this.match('pair');
    }
    return this.match('scalar');
};
