...
rules.block_expr = function() {
    ...
    if ( this.test('if_') ) {
        ...
        r = this.match('apply');
    //  FIXME: Важно, чтобы value шел перед compound. Иначе выражение вида (...) && (...) будет приводить к ошибке.
        ...
        this.backtrace('block_expr expected');
    }
...
//  inline_primary := inline_number | inline_string | inline_compound | root | jpath | inline_function | inline_var
...
rules.inline_primary = {
    ...
    rule: function(ast) {
        ...
        if (this.test('(')) {
            expr = this.match('inline_compound');
        } else if (this.test('/')) {
...
//  inline_compound := '(' inline_expr ')'

rules.inline_compound = {
