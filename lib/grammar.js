...
rules.module_name = function(parser, ast) {
    if ( parser.is_token('MODULE') ) {
        ...
        ast.Value = parser.match( 'inline_string', { noexpr: true } ).asString();
...
rules.block = function(parser, ast, params) {
    ...
    while ( !( parser.is_eof() || parser.is_token('}') || parser.is_token(']') || parser.is_token(')') ) ) {
        //  FIXME: Кажется, тут просто is_eol нужно вызвать?
...
rules.import = function(parser, ast) {
    ...
    ast.Filename = parser.match( 'inline_string', { noexpr: true } ).asString();
};
...
function match_typedef(parser) {
    ...
    //  FIXME: Тут парсится не тот токен, который в test_typedef.
...
rules.argtypes = function(parser, ast) {
    parser.token('(');
    if ( !parser.is_token(')') ) {
        //  FIXME: Сделать не массив, а items.
        ast.add( parser.match('argtype') );
        while ( parser.is_token(',') ) {
            parser.token(',');
            ast.add( parser.match('argtype') );
        }
...
rules.argtype = function(parser, ast) {
    ast.Type = match_typedef(parser);
};
...
yate.grammar.skippers.default_ = function(parser) {
    var r = false;
    while (1) {
        var l = parser.skip('spaces') || parser.skip('inline_comments') || parser.skip('block_comments') || parser.skip('html_comments');
        r = r || l;
        if (!l) { break; }
    }
    return r;
...
yate.grammar.skippers.spaces = function(parser) {
    if ( parser.is_eof() ) {
        return false;
    }
    ...
    var ch = parser.input.charCodeAt(0);

    if (ch === 32 || ch === 9) {
        var r = rx_spaces.exec(input);
        if (r) {
            parser.move( r[0].length );
        }
...
yate.grammar.skippers.whitespaces = function(parser) {
    parser.skip('spaces');
    while ( parser.is_eol() ) {
        parser.nextline();
        parser.skip('spaces');
...
yate.grammar.skippers.none = function(parser) {};

yate.grammar.skippers.inline_comments = function(parser) {
    if ( parser.is_eof() ) { return; }

    var input = parser.input;

    if (input.charCodeAt(0) === 47 && input.charCodeAt(1) === 47) {
        parser.move(input.length);

        return true;
    }
};

yate.grammar.skippers.block_comments = function(parser) {
    if ( parser.is_eof() ) { return; }

    var input = parser.input;

    if (input.charCodeAt(0) === 47 && input.charCodeAt(1) === 42) {
        parser.move(2);
...
yate.grammar.skippers.html_comments = function(parser) {
    if ( parser.is_eof() ) { return; }

    var input = parser.input;

    if (input.charCodeAt(0) === 60 && input.charCodeAt(1) === 33 && input.charCodeAt(2) === 45 && input.charCodeAt(3) === 45) {
        parser.move(4);
