...
rules.block = function(parser, ast, params) {
    ...
    var Items = ast.Items;
    console.log('BLOCK', parser.input);
    ...
    while ( !( parser.is_eof() || parser.is_token('}') || parser.is_token(']') || parser.is_token(')') ) ) {
        console.log('BLOCK LOOP', parser.input);
        ...
        Items.add(r);
        console.log('added', r.id, parser.input, parser.is_eol());
        ...
        parser.nextline();
        parser.skip();
        console.log('END OF LINE', parser.input);
    }
    console.log('END OF LOOP', parser.input);
};
...
rules.body = function(parser, ast) {
    parser.token('{');
    console.log('BODY 1', parser.input);
    ast.Block = parser.match('block');
    parser.token('}');
    console.log('BODY END', parser.input, parser.is_eol());
};
...
rules.template = function(parser, ast) {
    ...
    console.log('TEMPLATE END', parser.input, parser.is_eol());
};
...
yate.grammar.skippers.inlineComments = function(parser) {
...
yate.grammar.skippers.jsBlockComments = function(parser) {
    if ( parser.is_eof() ) { return; }
...
yate.grammar.skippers.htmlBlockComments = function(parser) {
    if ( parser.is_eof() ) { return; }
