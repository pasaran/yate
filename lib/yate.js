...
Yate.prototype.compile = function() {
    ...
    if (options.stdout) {
        var js = this.compile_to_string( options.files[0] );
        console.log(js);
    } else {
        this.compile_to_file();
    }
}

Yate.prototype.compile_to_string = function(filename) {
    var options = this.options;
    ...
    return this.compiler.compile(filename, params);
};

Yate.prototype.compile_to_file = function() {
    var options = this.options;

    var suffix = (options.target === 'node') ? '.node.js' : '.js';
    ...
    for (var i = 0, l = files.length; i < l; i++) {
        ...
        var js = this.compile_to_string(filename);
