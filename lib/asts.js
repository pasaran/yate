...
yate.asts.block.w_deinclude = function() {
    ...
    this.p.Items.iterate(function(item) {
        if (item.id === 'include') {
            var filename = path_.resolve(
                path_.dirname(item.where.input.filename),
                item.p.Name.asString()
            );
            var ast = yate.parse(filename, 'module');
...
yate.asts.string_literal.dbl_quote_escape = function() {
    return this.stringify().slice(1,-1);
};
