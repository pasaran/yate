...
asts.items.iterate = function(callback) {
    this.p.forEach(callback);
};

asts.items.grep = function(callback) {
    return this.p.filter(callback);
};
...
asts.items.mergeWith = function(ast) {
    this.p.push.apply(this.p, ast.p);
};
...
asts.inline_function.prepare = function() {
    ...
    if (def.f.IsExternal) {
        ...
        var defArgs = def.p.Args;
        args.iterate(function(arg, i) {
            arg.cast( defArgs.p[i].p.Typedef || 'scalar' );
        });
