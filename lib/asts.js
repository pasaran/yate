...
yate.asts.module.w_prepare = function() {
    ...
    var requires = this.requires = this.make('requires');

    if (this.externals) {
        for (var i = 0, l = this.externals.length; i < l; i++) {
            add_require( this.externals[i] );
        }
    }

    var is_node = (this.target === 'node');

    var imports = this.Block.imports;
    var modules = [];
    if (imports) {
        for (var i = 0, l = imports.length; i < l; i++) {
            var import_ = imports[i];
            if (is_node) {
                add_require(import_.filename);
            } else {
                modules.push(import_.name);
            }
        }
    }

    function add_require(filename) {
        var extname = that.rel_module_name(filename);

        requires.add( that.make(
            'require',
            {
                Filename: that.make('string_literal', {
                    Value: extname
                })
            }
        ) );
...
yate.asts.requires.options = {
    mixin: 'items'
};

yate.asts.requires.jssep__require = '\n';

yate.asts.require = {};

yate.asts.require.options = {
    props: 'Filename'
};
...
yate.asts.block.w_deinclude = function() {
    //  FIXME: Как это разрулить? Непонятно.
    //  Между asts.js и compiler.js циклическая зависимость.
    //
    if (!compiler) {
        compiler = new yate.Compiler();
    }
    var a = [];
...
yate.asts.block.w_deimport = function(include_dir) {
    if (!compiler) {
        compiler = new yate.Compiler();
    }
    ...
    this.Items.iterate(function(item) {
        if (item.id === 'import') {
            var module_name = find_module(item.Filename);
            if (!module_name) {
                item.error('Cannot find module: ' + item.Filename);
            }
            ...
            var ast = compiler.parse(module_name);

            var defs = ast.Block.Items.Items;

            var b = [];
            for (var i = 0, l = defs.length; i < l; i++) {
                var def = defs[i];
                var id = def.id;
                if ( id === 'var_' || id === 'func' || id === 'external' || id === 'key' ) {
    ...
    function find_module(filename) {
        for (var i = 0, l = include_dir.length; i < l; i++) {
            var dirname = include_dir[i];

            if ( !/\.yate$/.test(filename) ) {
                filename += '.yate';
            }

            var module_name = path_.join(dirname, filename);
            if ( fs_.existsSync(module_name) ) {
                return module_name;
            }
        }
    }
