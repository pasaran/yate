...
asts.block._init = function(exprs) {
    this.Imports = this.make('block_imports');
    this.Defs = this.make('block_defs');
    this.Templates = this.make('block_templates');
    this.Exprs = this.make('block_exprs', exprs);
};
...
asts.block.js__matcher = function() {
    ...
    this.Templates.iterate(function(item) {
        addToMatcher(item);
    });
    ...
    function addToMatcher(template) {
        ...
        template.Selectors.iterate(function(selector) {
            var step;
            if (selector.is('root')) {
                step = '';
            } else {
            ...
            stepTemplates.unshift('t' + template.Id);
            if (step == '*') {
                for (var t in modeTemplates) {
                    if (t != '*') {
                        modeTemplates[t].unshift('t' + template.Id);
                    }
                }
            }
        });
...
asts.jpath.extractDefs = function() {
    ...
    this.Id = jid; // Запоминаем id-шник.
    this.Key = key;
};
...
asts.template._init = function() {
    this.Selectors = this.make('template_selectors');
};
...
asts.template_selectors.jssep__selector_ref = ', ';
