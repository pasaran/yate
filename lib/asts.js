...
asts.items._get_type = function() {
    ...
    var currentType = items[0].get_type();

    for (var i = 1; i < l; i++) {
        var item = items[i];
        var nextType = item.get_type();
...
asts.items.get_scope = function() {
    ...
    for (var i = 1; i < l; i++) {
        scope = yate.Scope.commonScope( scope, items[i].get_scope() );
    }
    ...
    this.Items.iterate(function(item) {
        if (item.id === 'import') {
            ...
            for (var i = 0, l = defs.length; i < l; i++) {
                ...
                switch (ast.id) {
                    case 'var_':
                        ast.state.vid = ast.Id + 1;
                        break;
                    case 'function_':
                        ast.state.fid = ast.Id + 1;
                        break;
                    case 'key':
                        ast.state.kid = ast.Id + 1;
                }
            }
            a = b.concat(a);
...
asts.if_._get_type = function() {
    ...
    this.Elses.iterate(function(item) {
        type = yate.types.commonType( type, item.get_type() );
    });
    return type;
};
...
asts.inline_internal_function.findSignature = function(callargs) {
    ...
    function checkArgs(signature, callargs) {
        ...
        for (var i = 0, l = callargs.length; i < l; i++) {
            ...
            if ( !arg || !yate.types.convertable( callarg.get_type(), arg ) ) {
                return false;
            }
        }
...
asts.jpath_predicate.isMatchable = function() {
    return this.Expr.isLocal() || this.Expr.get_type() === 'boolean';
};

asts.jpath_predicate.w_set_types = function() {
    if (this.isLocal() || this.Expr.get_type() === 'boolean') {
...
asts.jpath_filter.get_scope = function() {
    return yate.Scope.commonScope( this.Expr.get_scope(), this.JPath.get_scope() );
};
