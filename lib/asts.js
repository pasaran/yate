...
asts.module.w_action = function() {
    this.Filename = this.pos.filename;

    this.Name = path_.basename(this.Filename);
    ...
    if (externals) {
        ...
        for (var i = 0, l = externals.length; i < l; i++) {
            var extname = this.externals[i];

            Externals.add( this.make(
                'external_file',
                {
                    Filename: path_.join(
                        path_.relative(dirname, path_.dirname(extname) ),
                        path_.basename(extname)
                    )
                }
            ) );
        }
...
asts.external_files.options = {
    mixin: 'items'
};

asts.external_file = {};

asts.external_file.options = {
    props: 'Filename'
};
...
asts.apply._get_type = no.value('xml');
...
asts.attr._get_type = no.value('attr');
...
asts.attrs_close._get_type = no.value('xml');
...
asts.attrs_open._get_type = no.value('xml');
...
asts.arglist_item.is_const = no.false;
...
asts.pair._get_type = no.value('pair');
...
asts.object._get_type = no.value('object');
...
asts.array._get_type = no.value('array');

asts.array.w_action = function() {
    this.Block.set_as_list();
};
...
asts.cdata._get_type = no.value('xml');
...
asts.inline_number.is_local = no.false;

asts.inline_number.is_const = no.true;

asts.inline_number._get_type = no.value('scalar');
...
asts.inline_string._get_type = no.value('scalar');
...
asts.string_content._get_type = no.value('scalar');
...
asts.string_literal._get_type = no.value('scalar');
...
asts.string_literal.is_local = no.false;
...
asts.inline_var.is_local = no.false;
...
asts.sort._get_type = no.value('nodeset');
...
asts.jpath._get_type = no.value('nodeset');
...
asts.jpath_filter._get_type = no.value('nodeset');
...
asts.simple_jpath._get_type = no.value('nodeset');
...
asts.xml._get_type = no.value('xml');
