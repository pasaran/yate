...
yate.run = function(yate_filename, data, ext_filename, mode, globals) {
    ...
    // Глобальные объекты, доступные во время шаблонизации из external функций.
    var sandbox = {
        console: console
    };

    globals = globals || {};
    for (var objName in globals) {
        sandbox[objName] = globals[objName];
    }

    var result = vm_.runInNewContext(js, sandbox);
