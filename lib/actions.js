...
yate.parse = function(filename) {
    var parser = new Parser(yate.grammar, yate.factory);
...
yate.compile = function(filename) {
    ...
    if (ast.p.Name) {
        ...
        console.log( ast.p.Block );
        var a = [];
        ast.p.Block.scope.defs.forEach(function(def) {
            if ( def.is('var_') || def.is('function_') || def.is('key') ) {
        ...
        fs_.writeFileSync(yobjFilename, JSON.stringify(obj, null, 4), 'utf-8');
    }
