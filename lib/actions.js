...
yate.parse = function(filename) {
    ...
    */
    ...
    if ( yate.cliOptions['write-ast'] ) {
        fs_.writeFileSync(yastFilename, AST.serialize(ast), 'utf-8');
    }
    */
...
yate.compile = function(filename) {
    ...
    //  console.time('walkdo.deinclude');
    ast.walkdo(function(ast) {
        ast.w_deinclude();
    });
    //  console.timeEnd('walkdo.deinclude');

    //  console.time('walkdo.deimport');
    ast.walkdo(function(ast) {
        ast.w_deimport();
    });
    //  console.timeEnd('walkdo.deimport');

    //  console.time('dowalk.deitemize');
    ast.dowalk(function(ast) {
        ast.w_deitemize();
    });
    //  console.timeEnd('dowalk.deitemize');

    var do_walk_list = [];
    var walk_do_list = [];
    //  console.time('build');
    function doo(ast, parent) {
        do_walk_list.push(ast);
    ...
    */
    ...
    for (var i = 0, l = do_walk_list.length; i < l; i++) {
        var item = do_walk_list[i];
    ...
    console.timeEnd('dowalk.scope');
    */

    //  Действие над каждой нодой в ast, не выходящее за рамки этой ноды и ее state/scope/context.
    //  console.time('walkdo.action');
    for (var i = 0, l = walk_do_list.length; i < l; i++) {
        walk_do_list[i].w_action();
    }
    /*
    ast.walkdo(function(ast) {
        ast.w_action();
    ...
    */
    //  console.timeEnd('walkdo.action');

    /*
    console.time('walk.list');
    ast.dowalk(function(ast) {
        ast.w_list();
    });
    console.timeEnd('walk.list');
    */
    ...
    //  console.time('dowalk.validate.types');
    for (var i = 0, l = do_walk_list.length; i < l; i++) {
        var item = do_walk_list[i];
    ...
    //  console.timeEnd('dowalk.validate.types');
    /*
    ...
    */
    ...
    //  console.time('walkdo.defs');
    for (var i = 0, l = walk_do_list.length; i < l; i++) {
        walk_do_list[i].w_extract_defs();
    }
    /*
    ast.walkdo(function(ast) {
        ast.w_extract_defs();
    ...
    */
    ...
    //  console.time('dowalk.prepare');
    for (var i = 0, l = do_walk_list.length; i < l; i++) {
        do_walk_list[i].w_prepare();
    }
    /*
    ast.dowalk(function(ast) {
        ast.w_prepare();
    ...
    */
    ...
    //  console.timeEnd('walkdo.transform');
