...
yate.AST.serialize = function(ast) {
    // return JSON.stringify( yate.AST.toJSON(ast), null, 4 );
...
yate.AST.prototype.toJSON = function(ast) {
    var r = {};
...
yate.AST.deserialize = function(obj) {
    ...
    var input = new InputStream(filename);
    ...
    for (var key in obj.p) {
        ...
        if (value instanceof Array) {
            ...
            for (var i = 0, l = value.length; i < l; i++) {
                r.push( yate.AST.fromJSON(value[i], input) );
            }
        } else {
