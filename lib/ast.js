...
var langs = {
    js: new Codegen( 'js', path_.join(templatesDir, 'js.tmpl') ),
    yate: new Codegen( 'yate', path_.join(templatesDir, 'yate.tmpl') )
};
...
AST.prototype._code = function(lang, mode) {
    ...
    var result = langs[lang].generate(data.id, data, mode);
    if (result !== undefined) {
        return result;
    }
