...
Yate.AST.prototype._fill = function(type, id, data) {
    ...
    return Yate.CodeTemplates.fill(type, id, data, type);
};
...
Yate.AST.prototype.children = function() {
    ...
    if (order) {
        ...
        for (var key in this) {
            if (this.hasOwnProperty(key) && /^\$/.test(key)) {
...
Yate.AST.prototype._getType = function() {
    this.error('Yate.AST.' + this.id + '.prototype._getType is not defined');
};
...
Yate.AST.prototype.log = function() {
    console.log(this.id, this.parent && this.parent.id, this.scope && this.scope.id);
};
