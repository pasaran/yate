...
yate.AST.prototype.rel_module_name = function(module_name) {
    var dirname = path_.dirname(this.pos.filename);

    module_name = path_.join(
        path_.relative(dirname, path_.dirname(module_name) ),
        path_.basename(module_name)
    );
    if ( !/^[./]/.test(module_name) ) {
