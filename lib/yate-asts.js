...
var Scope = require('./yate-scope.js');
...
asts.block._init = function(exprs) {
    this.Defs = this.ASTS.make('block_defs');
    this.Templates = this.ASTS.make('block_templates');
    this.Exprs = this.ASTS.make('block_exprs', exprs);
};
...
asts.inline_expr.codedata$ = function(lang) {
    if (this.AsType && lang !== 'yate') {
        var cast = this.ASTS.make( 'cast', { to: this.AsType, expr: this } );
...
asts.inline_function.action = function() {
    ...
    var def = this.def = ( this.scope.findFunction(name) || internalFunctions[name] );
    if (!def) {
...
(function() {
    ...
    for (var name in items) {
        internalFunctions[name] = factory.make( 'inline_internal_function', items[name] );
    }
})();
...
asts.block_exprs.yate$ = function() {
    var exprs = [];
    var indent = 0;
