...
var types = require('./yate-types.js');
var consts = require('./yate-consts.js');
...
asts.cast._init = function(to, expr) {
    this.From = expr.type();
    this.To = to;
    this.Expr = expr;
    this.mode = expr.mode;
...
asts.inline_complex.options = {
    base: 'inline_expr'
};

asts.inline_complex.isLocal = function() {
    return this.Expr.isLocal();
};

asts.inline_complex._getType = function() {
...
asts.inline_expr.codedata$ = function(lang) {
    if (this.AsType && lang !== 'yate') {
        ...
        //  Чтобы не зациклиться (cast.js() вызызовет inline_expr.js()), снимаем флаг о том,
...
asts.inline_internal_function._init = function(name, type, argTypes) {
    this.Name = name;
    this._type = type;
...
asts.jpath_filter._init = function(expr, jpath) {
    this.Expr = expr;
    this.JPath = jpath;
};
...
asts.jpath.extractDefs = function() {
    var key = this.common(); // Каноническая запись jpath.
...
asts.jpath_predicate.extractDefs = function() {
    var key = this.Expr.common(); // Каноническая запись предиката.
...
asts.quote._init = function(expr, mode) {
    this.Expr = expr;
    this.Mode = mode;
};
...
// См. codegen.js
asts.string_literal.common = function() {
    return this.Value;
};
...
asts.block.js$matcher = function() {
    var matcher = {};
...
asts.xml_line.js$content = function() {
    var items = [];
    this.toResult(items);
...
asts.block_exprs.common$ = function() {
    var exprs = [];
    var indent = 0;
    ...
    this.iterate(function(expr) {
        ...
        if (delta < 0) indent--;
        exprs.push( expr.common().replace(/^/gm, Array(indent + 1).join('    ')) );
        if (delta > 0) indent++;
    });
...
asts.block_templates.commonsep$ = '\n\n';

asts.block_defs.commonsep$ = '\n';

asts.arglist.commonsep$ = ', ';

asts.callargs.commonsep$ = ', ';
