...
yate.types.join_type = function(left, right) {
    //  NONE + ??? == NONE
...
yate.types.is_convertable = function(from, to) {
    return (
        (from == to) ||
        (to == 'any') ||
        (from == 'undef') ||
...
//  FIXME: Этот метод используется только в if_._get_type. Унести его туда.
yate.types.common_type = function(left, right) {
